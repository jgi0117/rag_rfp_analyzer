...
print("🚀 [시나리오-의미단위-A] ko-sroberta-multitask 연결 중...")
embeddings_semantic_a = HuggingFaceEmbeddings(
    model_name="jhgan/ko-sroberta-multitask", model_kwargs={'device': 'cpu'}, encode_kwargs={'normalize_embeddings': True}
)

persist_db_semantic_a = os.path.join(project_root_dir, "chroma_db_semantic_a")
vector_db = Chroma(persist_directory=persist_db_semantic_a, embedding_function=embeddings_semantic_a)
...
print("\n📊 [의미 단위 청킹 + ko-sroberta 로컬 실험 리포트]")

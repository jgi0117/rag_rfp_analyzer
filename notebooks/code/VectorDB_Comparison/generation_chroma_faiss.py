...
# (embedding_chroma_faiss.py와 동일한 함수 — 동작 기준 통일)
# =============================================================================
...
# 본 스크립트는 embedding 단계에서 생성된 DB를 로드해 Generation 통합 평가를 수행합니다.
parser = argparse.ArgumentParser(description="Generation evaluation experiment (Chroma / FAISS)")
...
# =============================================================================
# 2. 벡터 DB 로드 — Chroma / FAISS 분기
# =============================================================================

persist_db_b = resolve_project_path(config["retrieval"]["persist_directory"])
if not os.path.exists(persist_db_b):
    raise FileNotFoundError(
        f"Vector DB directory not found: {persist_db_b}\n"
        "먼저 embedding_chroma_faiss.py를 실행해 DB를 생성해 주세요."
    )

if vector_db_type == "faiss":
    vector_db_b = FAISS.load_local(
        persist_db_b,
        embeddings_b,
...
print(f"Vector DB loaded ({vector_db_type}): {persist_db_b}")

# =============================================================================
# 3. Ground Truth 기반 Retrieval 검색
# =============================================================================
...
# retrieval 규격 컬럼을 generation_eval_metrics 규격으로 변환
...
# 검색 컨텍스트를 취합해 모델 답변 생성
# 첫 번째 질문은 GPU 워밍업으로 인한 시간 편향이 있으므로 로그에 표시

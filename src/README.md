...
- `retriever.py`: 향후 검색 기능을 독립 모듈로 분리하기 위한 파일입니다.
- `generator.py`: 향후 답변 생성 기능을 독립 모듈로 분리하기 위한 파일입니다.
...
- `retriever.py`와 `generator.py`는 현재 비어 있는 확장 지점입니다. 검색/생성 로직이 복잡해지면 baseline 스크립트에서 이 파일들로 옮겨 구조화할 수 있습니다.
